 ...
 */
const CircleCard = ({ slug, title, shortDescription, imageSrc, link }: CircleData) => (
  ...
  <article
    ...
    tabIndex={0}
    className="group flex flex-col snap-center min-w-full md:min-w-92.5 min-h-137 overflow-hidden rounded-xl p-0 select-none bg-neutral-200 hover:bg-neutral-300 focus-ring transition-all duration-300"
  >
    ...
    <div className="flex flex-col justify-between items-center md:items-start grow p-6 gap-4">
      ...
      <h3
        id={`${slug}-title`}
        className="font-bold text-xl md:text-2xl text-primary-500"
      >
        {title}
      </h3>

      {/* Truncated description with relaxed line height and max width for readability */}
      <p className="max-w-77.5 leading-relaxed line-clamp-4 text-neutral-900">
        {shortDescription}
      </p>

      {link && (
        // Optional roadmap CTA button with icon
        <Link
          href={link}
