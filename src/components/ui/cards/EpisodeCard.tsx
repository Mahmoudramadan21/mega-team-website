...
import PodcastRoundedImage from '../images/PodcastRoundedImage';
    ...
    >
      ...
      {isBackground && (
        <>
          ...
          <BackgroundImage
            imageSrc={props.images.left}
            position="left"
          />
          {/* Decorative background image - bottom right corner */}
          <BackgroundImage
            imageSrc={props.images.right}
            position="right"
          />
        </>
      )}
      ...
      <div className={`relative z-10 flex flex-col items-center md:items-start gap-3 mx-auto ${isBackground ? "max-w-80" : ""}`}>
        {/* Episode meta information (number and date) */}
        <div className="font-semibold text-sm text-primary-500">
          <span>Episode {episode} • </span>
          <time dateTime={date} aria-label={`Episode date: ${date}`}>{date}</time>
        </div>

        {/* Episode title with unique ID for accessibility reference */}
        <h3 id={`episode-${episode}-title`} className="font-bold text-xl md:text-2xl text-neutral-50">
          {title}
        </h3>

        {/* Truncated episode description with relaxed line height */}
        <p className="leading-relaxed line-clamp-3 text-neutral-300">
          {description}
        </p>

        {/* Play / Read More CTA button with icon and hover expansion */}
        <Link
